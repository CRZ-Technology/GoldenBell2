...
NVME_SD_REQUEST_QUEUE nvmeSdReqQ;

unsigned int notCompletedNvmeSdReqCnt;
...
void InitReqPool()
{
	...
	nvmeSdReqQ.headReq = REQ_SLOT_TAG_NONE;
	nvmeSdReqQ.tailReq = REQ_SLOT_TAG_NONE;
	nvmeSdReqQ.reqCnt = 0;
...
void PutToNvmeSdReqQ(unsigned int reqSlotTag)
{
	if(nvmeSdReqQ.tailReq != REQ_SLOT_TAG_NONE)
	{
		reqPoolPtr->reqPool[reqSlotTag].prevReq = nvmeSdReqQ.tailReq;
		reqPoolPtr->reqPool[reqSlotTag].nextReq = REQ_SLOT_TAG_NONE;
		reqPoolPtr->reqPool[nvmeSdReqQ.tailReq].nextReq = reqSlotTag;
		nvmeSdReqQ.tailReq = reqSlotTag;
	}
	else
	{
		reqPoolPtr->reqPool[reqSlotTag].prevReq = REQ_SLOT_TAG_NONE;
		reqPoolPtr->reqPool[reqSlotTag].nextReq = REQ_SLOT_TAG_NONE;
		nvmeSdReqQ.headReq = reqSlotTag;
		nvmeSdReqQ.tailReq = reqSlotTag;
	}

	reqPoolPtr->reqPool[reqSlotTag].reqQueueType = REQ_QUEUE_TYPE_NVME_SD;
	nvmeSdReqQ.reqCnt++;
	notCompletedNvmeSdReqCnt++;
}

void GetFromNvmeSdReqQ(void)
{
	unsigned int reqSlotTag;

	reqSlotTag = nvmeSdReqQ.headReq;
	if(reqSlotTag == REQ_SLOT_TAG_NONE)
		assert(!"[WARNING] there is no request in Nand-req-queue[WARNING]");

	if(reqPoolPtr->reqPool[reqSlotTag].nextReq != REQ_SLOT_TAG_NONE)
	{
		nvmeSdReqQ.headReq = reqPoolPtr->reqPool[reqSlotTag].nextReq;
		reqPoolPtr->reqPool[reqPoolPtr->reqPool[reqSlotTag].nextReq].prevReq = REQ_SLOT_TAG_NONE;
	}
	else
	{
