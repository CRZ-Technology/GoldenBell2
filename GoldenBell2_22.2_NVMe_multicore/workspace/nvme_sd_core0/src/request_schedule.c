...
{
	while((nvmeDmaReqQ.headReq != REQ_SLOT_TAG_NONE) || notCompletedNvmeSdReqCnt || blockedReqCnt)
	{
		CheckDoneNvmeDmaReq();
		CheckDoneNvmeSdReq();
	}
...
void CheckDoneNvmeSdReq(void)
{
	ProcessNvmeSdResPerCh();
}

void ProcessNvmeSdResPerCh(void)
{
    ExecuteNvmeSdRes();
}

void IssueNvmeSdRes(void)
{
    ProcessNvmeSdRes();
}
...
{
	if(reqPoolPtr->reqPool[reqSlotTag].reqType == REQ_TYPE_NVME_SD)
	{
