...
void nvme_main(void)
{
    ...
    while(*(unsigned int *)start_res_addr == 0)
        *(unsigned int *)start_req_addr = 1;

    nvme_sd_storage = *(unsigned int *)storage_info_addr;
	storageCapacity_L = nvme_sd_storage / (BYTES_PER_NVME_BLOCK / BYTES_PER_SD_BLOCK);
	...
	while(1)
	{
		...
		if((nvmeDmaReqQ.headReq != REQ_SLOT_TAG_NONE) || notCompletedNvmeSdReqCnt || blockedReqCnt)
		{
			CheckDoneNvmeDmaReq();
			CheckDoneNvmeSdReq();
		}
